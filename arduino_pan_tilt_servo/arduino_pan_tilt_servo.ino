...
void loop() {
  ...
  // tilt downwards 
  for (top_pos = 90; top_pos <= 180; top_pos += 1) { // goes from 0 degrees to 180 degrees in steps of 1 degree
  ...
  // tilt upwards 
  for (top_pos = 180; top_pos >= 90; top_pos -= 1) { // goes from 180 degrees to 0 degrees
    top_servo.write(top_pos);                 // tell servo to go to position in variable 'pos'
    delay(10);                          // waits 15ms for the servo to reach the position
  }
  delay(1000);
